...
public abstract class ImageProcessing {
    ...
    private static int[] decodeYUV420SPtoSum(byte[] yuv420sp, int width, int height) {
        int[] res = new int[2];
        if (yuv420sp == null) return res;
    ...
    public static int[] decodeYUV420SPtoAvg(byte[] yuv420sp, int width, int height) {
        ...
        if (yuv420sp == null) return sumArr;

        final int frameSize = width * height;

        sumArr = decodeYUV420SPtoSum(yuv420sp, width, height);
        sumArr[0] = sumArr[0] / frameSize;
        sumArr[1] = sumArr[1] / frameSize;
        return sumArr;
    }
}
